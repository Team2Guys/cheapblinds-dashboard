...
export const dataProvider: DataProvider = {
  getList: async ({ resource, meta }) => {
    if (!meta?.gqlQuery || !meta?.operationName) {
      throw new Error(
        `gqlQuery and operationName are required in meta for resource ${resource}`
      );
    }
    const { data } = await gqlClient
      .query(meta.gqlQuery, meta.variables || {})
      .toPromise();
  ...
  create: async ({ resource, variables, meta }) => {
    ...
    const { data } = await gqlClient
      .mutation(meta.gqlMutation, { input: variables })
      .toPromise();
